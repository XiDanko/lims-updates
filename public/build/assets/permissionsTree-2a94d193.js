...
`)});function U(a,s,c,b){var g=c?"":b.css;if(a.styleSheet)a.styleSheet.cssText=S(s,g);else{var m=document.createTextNode(g),I=a.childNodes;I[s]&&a.removeChild(I[s]),I.length?a.insertBefore(m,I[s]):a.appendChild(m)}}},function(x,k){x.exports=function(f){var o=typeof window<"u"&&window.location;if(!o)throw new Error("fixUrls requires window.location");if(!f||typeof f!="string")return f;var i=o.protocol+"//"+o.host,u=i+o.pathname.replace(/\/[^\/]*$/,"/");return f.replace(/url\s*\(((?:[^)(]|\((?:[^)(]+|\([^)(]*\))*\))*)\)/gi,function(y,w){var l,e=w.trim().replace(/^"(.*)"$/,function(t,n){return n}).replace(/^'(.*)'$/,function(t,n){return n});return/^(#|data:|http:\/\/|https:\/\/|file:\/\/\/|\s*$)/i.test(e)?y:(l=e.indexOf("//")===0?e:e.indexOf("/")===0?i+e:u+e.replace(/^\.\//,""),"url("+JSON.stringify(l)+")")})}}]).default})})(Q);(function(C){C.exports=R})(K);const Y=H(A),Z={id:"treeselect"},te=J({__name:"Treeselect",props:{modelValue:null,options:null},emits:["update:modelValue"],setup(C,{emit:O}){const x=C;let k=null;W(()=>{f()});const f=()=>{k=new Y("#treeselect",{data:x.options,values:x.modelValue,closeDepth:1,onChange:function(){O("update:modelValue",this.values)}})};return F(()=>x.modelValue,o=>{o===void 0&&(k.values=[])}),(o,i)=>(G(),$("div",Z))}}),ne=()=>{const C=P([]),O=P([]),x=async()=>{const{data:o}=await X.get("/api/permissions/list");O.value=o},k=async()=>{await x();let o=O.value.map(i=>i.split(".")[0]);return o=[...new Set(o)],o.forEach(i=>{C.value.push(f(i))}),C.value},f=o=>{const i={id:o,text:V.startCase(o),children:[]};return O.value.filter(y=>y.startsWith(o+".")).forEach(y=>i.children.push({id:y,text:V.startCase(y.split(".")[1])})),i};return{getPermissionsTree:k}};export{te as _,ne as u};
